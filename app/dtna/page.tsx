...
export default function DtnaPage(){
  ...
  async function refresh(){
    try{
      ...
      setWorker(null);setStatus(null);setOnline(false);setLoginInitialized(false);setMsg('Run the Diehl initializer once on this Windows computer.');
    }
  }
  ...
  async function initializeLogin(){
    try{
      setMsg('Opening DTNA locally. Sign in with YOUR DTNA username/password and complete MFA.');
      await local('/dtna/open',{method:'POST'});
  ...
  async function runSync(){
    ...
    try{setMsg('Starting Sales Order + AUTO VIN sync locally…');await local('/dtna/sync',{method:'POST'});setMsg('Sales Order + AUTO VIN sync started locally.')}catch(e:any){setMsg(e.message)}
  }
  ...
  return <main className="dtna-page">
    ...
    <section className="dtna-grid">
      <article className="dtna-card"><h2>This computer</h2><dl><dt>Computer</dt><dd>{worker?.hostname||'—'}</dd><dt>Workbook</dt><dd>{worker?.master_workbook||'—'}</dd><dt>DTNA login</dt><dd>{loginInitialized?'Initialized for this user':'Required'}</dd><dt>Persistent profile</dt><dd>{status?.profile||'—'}</dd></dl></article>
