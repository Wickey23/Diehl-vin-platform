...
const PACKAGE_REF = '8516728e9cb604c8e40dc752bf99b09f6b1132c9';
const FILES = [
...
export async function GET() {
  try {
    ...
    if (!launcher.includes('SETUP AND START v5.3')) throw new Error('Pinned v5.3 launcher verification failed.');
    ...
    if (!database.includes("version='1.2'") || !database.includes('/control/stop-all') || !database.includes('_find_open_workbook') || database.includes('from openpyxl import load_workbook')) {
      throw new Error('Pinned lock-free database viewer verification failed.');
    }

    const zip = new JSZip();
    const folder = zip.folder('Diehl_VIN_Local_Worker_v5_3');
    ...
    folder.file('PACKAGE VERSION.txt', [
      ...
      'Expected launcher banner: DIEHL VIN - SETUP AND START v5.3',
      ...
      'Database viewer reads the live workbook through Excel COM only and does not open the xlsx directly with openpyxl.',
    ...
    folder.file('READ ME FIRST.txt', [
      ...
      '4. The banner must say DIEHL VIN - SETUP AND START v5.3.',
      ...
      '7. You may leave the shared workbook open in Excel. Database reads and DTNA writes attach through Excel instead of directly locking the xlsx file.',
      ...
      '10. The website Database tab reads DTNA and VIN In-Service through the local Excel COM database service.',
      '',
    ...
    return new Response(body, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Diehl_VIN_Local_Worker_v5_3.zip"',
