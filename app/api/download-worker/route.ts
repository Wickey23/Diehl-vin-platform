...
const PACKAGE_REF = '2980685afe1db6c0fc1a62e7e082a60eedb00d81';
const FILES = [
...
export async function GET() {
  try {
    ...
    if (!launcher.includes('SETUP AND START v5.7') || !launcher.includes('database_cache.py') || !launcher.includes('excel_bridge.py')) throw new Error('Pinned v5.7 launcher verification failed.');
    ...
    const zip = new JSZip();
    const folder = zip.folder('Diehl_VIN_Local_Worker_v5_7');
    ...
    folder.file('READ ME FIRST.txt', [
      ...
      '4. The banner must say DIEHL VIN - SETUP AND START v5.7.',
      '5. VIN In-Service opens OWL at secure.freightliner.com/iwarranty/signOn.',
      '6. Coverage Info uses the exact WarrantyDetailsGoToServlet page supplied for in-service/warranty information.',
      '7. Major Components uses the exact ProductMaintenanceServlet page supplied for engine/Allison serial numbers.',
      ...
      '11. Open Excel activates or opens DIEHL-VIN-PLATFORM WORKBOOK.xlsx itself, not a blank workbook.',
    ].join('\r\n'));
    ...
    return new Response(body, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Diehl_VIN_Local_Worker_v5_7.zip"',
