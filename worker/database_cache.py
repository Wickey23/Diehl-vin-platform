...
def write_table(sheet: str, headers: list[str], rows: list[dict[str, Any]], workbook: str, source: str) -> None:
    ...
    payload = {
        ...
        'exists': True,
        'modified': time.time(),
        'readMode': 'DTNA/OWL local mirror',
        'source': source,
    }
...
def update_vin(vin: str, result: dict[str, Any], workbook: str) -> None:
    ...
    with LOCK:
        ...
        write_table(sheet, headers, rows, workbook, 'OWL exact-field mapper -> shared Excel save')


def refresh_dtna_from_output(workbook: str = '') -> bool:
    ...
    try:
        ...
        if not headers:
            return False
        write_table('DTNA', headers, rows, workbook, 'DTNA Sales Order + AUTO VIN merged output')
        return True
    except Exception:
        return False
...
def _dtna_output_is_newer(target: Path) -> bool:
    if not DTNA_OUTPUT.exists():
        return False
...
def read_table(sheet: str, limit: int = 10000, seed: bool = True) -> dict[str, Any]:
    if sheet not in ALLOWED:
        raise ValueError(f'Unsupported database sheet: {sheet}')
    target = _cache_path(sheet)
    if seed and sheet == 'DTNA' and _dtna_output_is_newer(target):
        refresh_dtna_from_output()
    ...
    if not target.exists():
        return {
            ...
            'modified': 0,
            'readMode': 'DTNA/OWL local mirror',
            'source': 'No successful DTNA/OWL collection mirrored yet',
        }
    with LOCK:
