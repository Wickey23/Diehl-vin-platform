...
export default function Checker(){
  ...
  async function refresh(id=batch?.id){
    ...
    if(!reachable){setWorker(null);setOnline(false);setMsg('Local Diehl worker is not running. Press START DIEHL VIN on this computer.');return}
    setOnline(true);const h=await healthDetails();if(h?.worker)setWorker(h.worker);
  ...
  async function start(){
    setBusy(true);setMsg('Starting live OWL lookup on this computer…');
    try{const d=await local('/batches',{method:'POST',body:JSON.stringify({vins:text,lookupMode:mode,workers,batchSize:100,retryRounds:3,batchPause:.5,retryPause:3})});setBatch({id:d.batchId,status:'queued',total_vins:d.total,lookup_mode:mode,created_at:new Date().toISOString(),options:{}});setItems([]);setText('');setMsg('Started — OWL Coverage Info and Major Components are processing the VINs');await refresh(d.batchId)}catch(e:any){setMsg(e.message)}finally{setBusy(false)}
  }
  ...
  return <div className="checker">
    <div className="hero"><div><div className="brandline">VIN IN-SERVICE CHECKER · FREIGHTLINER / OWL</div><h1>VIN In-Service</h1><p>Each VIN is checked in OWL Coverage Info / Check Coverage and Major Components, then written to the shared Excel database. Your Freightliner/OWL session stays local.</p></div><div className="pill">{online?'● This computer ready':'○ Initializer required'}</div></div>
    <div className="layout">
      <section className="card"><div className="cardhead"><span>VIN list</span><span className="muted">{vins.length} VINs</span></div><div className="pad">
        ...
        <div className="row" style={{marginTop:12}}><button className="primary wide" disabled={busy||!vins.length||!online} onClick={start}>{busy?'Starting…':'Start OWL Check'}</button><button className="ghost" onClick={()=>setText('')}>Clear</button></div>
        ...
        <div className="workerbox"><b>This computer</b><p className="help">{online?`${worker?.hostname||'Local worker'} · ${worker?.master_workbook||'Worker connected'}`:'Press START DIEHL VIN on this computer.'}</p><button className="ghost" onClick={openOwl} disabled={!online}>Open OWL / Login</button></div>
      </div></section>
