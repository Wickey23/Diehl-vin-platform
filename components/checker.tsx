...
export default function Checker(){
  ...
  async function start(){
    ...
    try{
      ...
      setBatch({id:d.batchId,status:'queued',total_vins:d.total,lookup_mode:mode,created_at:new Date().toISOString(),options:{}});setItems([]);setText('');setMsg('Started — OWL is processing the VINs on this computer');await refresh(d.batchId)
  ...
  async function openOwl(){try{await local('/owl/open',{method:'POST'});setMsg('OWL opened locally. Complete your DTNA login/MFA if requested, then leave OWL Vehicle/VIN Search ready.')}catch(e:any){setMsg(e.message)}}
  ...
  return <div className="checker">
    <div className="hero"><div><div className="brandline">VIN IN-SERVICE CHECKER · FREIGHTLINER / OWL</div><h1>VIN In-Service</h1><p>Every submitted VIN is checked live in OWL on this computer, then written to the shared Excel database. Your DTNA/OWL session stays local.</p></div><div className="pill">{online?'● This computer ready':'○ Initializer required'}</div></div>
